...
5. **Anti-Clutter & Zero-Permanent-Storage Invariant:** Educational attachments (.pdf, .jpg, .docx, .pptx, .mp3, .mp4, media) downloaded during crawls are strictly transient. They are routed out of public shared storage (`Downloads/`, `Documents/`) into private sandbox staging (`vault_attachments/`), uploaded to Google Drive, and immediately deleted from local device storage upon confirmed upload. Public directories stay spotless, and in-app cache bloat from "Save all files offline" is intentionally bypassed.
...
- **Exhaustive Multi-Attachment Ingestion & Sequential Fresh-Node Re-Querying Loop:**
  ...
  3. **Sequential Fresh-Node Re-Querying Loop:**
     ...
     for ((index, fileName) in pendingTargetFileNames.withIndex()) {
         ...
         // Dynamic Boundary-Aware Detail Search with Physical Touch Gestures (Up to 6 Sweeps)
         if (targetChip == null) {
             // Phase 1: Downward physical touch gesture sweeps to reveal chips below the fold
             var previousBottomFingerprint = ""
             var scrollCount = 0
             while (targetChip == null && scrollCount < 6 && serviceScope.isActive && crawlerOverlay?.isAutoScrollingActive() == true) {
             ...
             // Phase 2: If chip was not below, rewind upward using physical gestures (up to 6 sweeps)
             if (targetChip == null) {
  ...
  - **Automated In-App Viewer & Preview Detection (`automateViewerShareOrDownload`):**
    ...
    2. **Viewer & Comments Screen Detection & Exclusion Invariant (`isDocumentViewerScreen`, `isCommentsOnlyScreen` vs. `isPostDetailView`):**
     ...
     3. **Prioritized 3-Dot Overflow Menu ("Send a Copy"):** To avoid the Google Drive collaborator invite modal ("Add people or groups") triggered by top toolbar "Share" icons, the crawler prioritizes clicking the 3-dot overflow menu button (`findOverflowMenuButton`), awaiting the popup menu (400ms), and selecting **"Send a copy"** (`findExplicitCopyOrDownloadButton`). Direct buttons are only evaluated as a fallback if no overflow menu exists.
     4. **Instant Post Detail Return Detection (Unshareable Media Invariant):** When an educational attachment cannot be shared due to school domain restrictions (e.g. view-only audio/video), Android does not display a share sheet; instead, clicking "Send a copy" causes the viewer to finish immediately and return focus directly to the Post Detail screen. The crawler inspects `isPostDetailView(rootInActiveWindow)` immediately after triggering "Send a copy": if already back on the Post Detail screen, it logs the domain restriction, marks the attachment as restricted in Room DB, skips the phantom share sheet search, and advances directly to the next attachment without dispatching any Back presses.
     5. **Guarded System Chooser Selection:** If a system share sheet does appear, it invokes `selectKidsInSystemChooser()`. If K.I.D.S. Vault is not present, it only dismisses the share sheet if the active window is genuinely the share sheet, strictly avoiding Back presses if the detail view is active.
     6. **Guarded Post Detail Retention Invariant:** When dismissing or returning from a document viewer, the crawler inspects `isPostDetailView(cur)`: if the Post Detail screen is already active, it breaks immediately without dispatching Back, guaranteeing that sibling attachments (e.g. worksheets paired with audio files) are never scrolled out of view.
