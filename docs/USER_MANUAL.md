...
- **Bidirectional Detail View Scanning for Multi-Attachment Notices:** When an announcement contains extensive paragraphs of text or multiple attachments (e.g. 5 to 15 PDFs and worksheets), Google Classroom pushes attachment chips below the fold. K.I.D.S. performs **Bidirectional Detail View Scanning**: it first scans downward within the detail screen (`performDetailScrollDown`), and if an attachment chip is not immediately found or after scrolling down, it smoothly rewinds upward (`performDetailScrollUp`) to locate and capture all attachment chips before returning to the stream.
- **Universal Share Sheet Target Matching:** In the system share sheet (`chooser`), K.I.D.S. recognizes all label variations of "K.I.D.S. Vault" (`kidsvault`, `kids`, `kidscollector`, `collector`), guaranteeing instant, error-free share target selection across all Android OEM skins.

#### 3. Intelligent Viewer Whitelisting & SystemUI Deadlock Self-Healing
