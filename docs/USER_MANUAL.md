...
- **Deep Multi-Swipe Detail View Scanning for Multi-Attachment Notices:** When an announcement contains multiple attachments (e.g. 5 to 15 PDFs and worksheets spanning several screen heights), Google Classroom stacks them in a long scrollable list. K.I.D.S. performs **Deep Multi-Swipe Scanning**: it scans downward up to 3 discrete swipes with a 400ms settling pause per swipe to allow Android `RecyclerView` item binding to complete, locating below-the-fold attachment chips. If a chip is not found below, it smoothly rewinds upward (up to 3 swipes) to reset its search position.
...
- **Universal Share Sheet Target Matching & Deep App List Search:** In the system share sheet (`chooser`), K.I.D.S. recognizes all label variations of "K.I.D.S. Vault" (`kidsvault`, `kids`, `kidscollector`, `collector`) and executes up to 4 search stages across horizontal and vertical app lists to ensure reliable target selection.
