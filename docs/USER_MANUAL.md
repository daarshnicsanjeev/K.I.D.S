...
- **Universal Multi-Format Media Support:** K.I.D.S. recognizes and ingests all standard educational classroom media, including documents (`.pdf`, `.docx`, `.xlsx`, `.pptx`, `.txt`, `.csv`, `.epub`), audio recordings (`.mp3`, `.m4a`, `.wav`, `.aac`, `.ogg`, `.wma`, `.flac`), videos (`.mp4`, `.mov`, `.avi`, `.mkv`, `.webm`), images (`.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`), and compressed archives (`.zip`, `.rar`, `.7z`), automatically mapping them to appropriate cloud MIME types and staging them seamlessly.
- **Dynamic Boundary-Aware Share Sheet Target Search:** In Android's system share sheet (`chooser`), K.I.D.S. executes upward expansion swipes to expose the complete multi-row application grid, honors a 750ms settling interval, and recognizes all label variations of "K.I.D.S. Vault" (`kidsvault`, `kids`, `kidscollector`, `collector`) before dynamically traversing the grid until K.I.D.S. Vault is selected.
- **High-Capacity 200-Character Filename Resolution:** Accommodates lengthy, descriptive school worksheet and answer key filenames up to 200 characters without truncation, preserving file extensions and key indicators.
...
6. **Autonomous Attachment Ingestion via Native Share Target ("Share to K.I.D.S. Vault"):**
   ...
   - **The True Autonomous Native Share Target Pipeline:**
     ...
     6. **Patient 7-Second Chooser Search & Bottom-Sheet Deep Scroll:**
        Android system share sheets can take several seconds to populate targets on memory-constrained devices or large app catalogs. K.I.D.S. enforces a **Patient 7-Second Share Sheet Search**:
        - **Extended 7,000ms Observation Window:** The assistant waits patiently for up to 7,000ms with 250ms polling intervals for the system chooser to render and locate K.I.D.S. Vault. As long as the file is not directly closed back to the post detail screen, K.I.D.S. will never rush or give up prematurely.
        - **Instant Return Handling on Domain Restrictions:** If school domain policy prohibits sharing a specific file (e.g. view-only audio/MP3 or domain-locked documents), Android displays no share sheet and the viewer finishes immediately directly back to the post detail screen. K.I.D.S. detects this instant return within milliseconds, marks the restriction in Room DB (`restricted_domain_policy`), skips share sheet scanning, and immediately continues to the next attachment without any accidental Back presses!
        - **Autonomous Drag-to-Expand ($0.85h \to 0.15h$):** If "K.I.D.S. Vault" is not visible in the initial preview sheet, the assistant dispatches an upward drag gesture ($0.50w, 0.85h \to 0.50w, 0.15h$) over 450ms, expanding the bottom sheet into a full multi-row app grid.
        - **Deep 5-Attempt Scroll Search:** It then executes up to 5 thorough scroll sweeps ($0.50w, 0.75h \to 0.50w, 0.25h$) across the app tiles with 750ms settling delays, ensuring K.I.D.S. Vault is discovered no matter how far down the OEM share sheet places it.
   ...
   - **Exhaustive Multi-Attachment Ingestion (Physical Touch Gesture Sweeps & Fresh-Node Loop):**
     In many educational posts—especially exam revision packs, unit test syllabi, or weekly homework summaries—a single notice may contain **multiple attachments (e.g. 10 practice worksheets, reading passages, and answer keys in a single post)**.
     ...
     2. *Below-the-Fold Omissions:* When more than 2–3 attachments exist, Classroom pushes remaining files below the viewport fold. Programmatic container scrolling (`ACTION_SCROLL_FORWARD`) often fails silently on Google Classroom's custom nested RecyclerViews.
     
     K.I.D.S. solves this with an **Exhaustive Multi-Attachment Ingestion Pipeline**:
     - **Physical Touch Gesture Sweeps (Up to 6 Downward Swipes):** Upon entering the post detail screen, K.I.D.S. extracts immediately visible attachments and then executes autonomous physical touch gesture sweeps downward (`performDetailScrollDown`) up to **6 times**. Any attachment chip positioned below the fold (e.g., worksheets 4 through 10) is exposed, discovered, deduplicated by filename, and registered in local SQLite Room (`AttachmentEntity`) with its unique file hash (`${noticeId}_${fileName}`).
     ...
     - **Sequential Fresh-Node Re-Querying Loop:** K.I.D.S. extracts the list of pending target filenames and iterates through them systematically one by one:
       ...
       4. *Adaptive Physical Touch Gesture Navigation:* If the chip has scrolled out of view, the crawler automatically scrolls the detail view downward using physical touch gestures (up to 6 times) to locate it, and rewinds upward using physical gestures (up to 6 times) if needed.
     ...
     - **Multi-Pass Auto-Recovery Engine (Guaranteed 100% Attachment Completion):**
       A crawl cycle is **NOT full or complete** until **100% of attachments are physically downloaded and synced to your Google Drive Vault**.
       If even a single attachment remains uncaptured (with the sole exception of domain-restricted view-only files where the share sheet is disallowed by the school domain), K.I.D.S. marks the cycle as **INCOMPLETE**:
       1. Automatically scans local staging storage and cross-references Room database for any notices with pending attachments (`att.localUri.isBlank() && !att.driveFileId.startsWith("restricted_")`).
       2. Resets those specific notices in the `StreamManifest` (`manifest.resetItemsForRecovery(incompleteTitles)`).
       3. Triggers an autonomous **Auto-Recovery Pass** (up to 4 passes), fast-forwarding directly to those posts and harvesting the missing attachments until 100% vault synchronization is attained!
     - **100% Ingestion Guarantee:** All attachments—even posts with 10+ heavy files—are completely captured, staged into private storage, and queued for Google Drive backup with zero dropped files and zero stale node crashes.
